	...
	<div class="user-view">
		...
		<div class="table-box">
			<div class="btn-box">
				...
				<el-button type="success" size="mini" @click="importUser">批量导入</el-button>
			</div>
			<el-table border :data="tableData.list" @selection-change="selectionChange">
				...
				<el-table-column min-width="70" label="昵称" prop="nickName"></el-table-column>
				<el-table-column min-width="90" label="手机号码" prop="phoneNumber"></el-table-column>
				<el-table-column min-width="100" label="所在企业" prop="companyName"></el-table-column>
		...
		<el-dialog title="添加用户" :visible.sync="dialogA.flag" width="600px">
			<el-form label-width="90px" size="mini" class="form-col2">
				...
				<el-form-item label="昵称"><el-input v-model="dialogA.form.nickName"></el-input></el-form-item>
				<el-form-item label="手机号码"><el-input v-model="dialogA.form.phoneNumber"></el-input></el-form-item>
				<el-form-item label="微信"><el-input v-model="dialogA.form.wechatNumber"></el-input></el-form-item>
				<el-form-item label="公司"><el-input v-model="dialogA.form.companyName"></el-input></el-form-item>
				<el-form-item label="职务"><el-input v-model="dialogA.form.position"></el-input></el-form-item>
...
export default {
	...
	methods: {
		getTableData() {
			this.$api.getUserList(this.form)
				.then((r) => {
					if(r.status === '200') {
		...
		showDialogA(n, row) {
			...
			if (n === 0) {//add
				this.dialogA.form = {
					id: '', name: '', nickName: '', phoneNumber: '', companyName: '',
					position: '', wechatNumber: '', type: 1, headUrl: ''
				}
			} else{//modify
				this.dialogA.form = {
					id: row.id, name: row.name, nickName: row.nickName, 
					phoneNumber: row.phoneNumber, companyName: row.companyName,
					position: row.position, wechatNumber: row.wechatNumber, 
					type: row.type, headUrl: row.headUrl
				}
			}
		},
		...
		saveUser() {
			...
			this.$api.userAOM(this.dialogA.form)
				.then((r)=>{
					if (r.status === '200') {
