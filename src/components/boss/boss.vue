	...
	<div class="hear-view">
		...
		<div class="table-box">
			...
			<el-table border :data="tableData.list" @selection-change="selectionChange">
				...
				<el-table-column min-width="60" label="分享图片" class-name="col-img">
					<template slot-scope="scope">
						<img :src="scope.row.shareImgUrl" width="35" height="35"/>
					</template>
				</el-table-column>
			...
			<el-form label-width="90px" size="mini" class="form-col2">
				...
				<el-form-item label="分享图片">
					<el-upload 
						...
						:before-upload="beforeAvatarUpload">
						<img v-if="dialogA.form.shareImgUrl" :src="dialogA.form.shareImgUrl" class="avatar">
						<i v-else class="el-icon-plus avatar-uploader-icon"></i>
					</el-upload>
				</el-form-item>
			</el-form>
			<quill-editor 
				v-model="dialogA.form.content" 
				ref="myQuillEditor" 
				class="editor" 
...
export default {
	...
	methods: {
		getTableData() {
			this.$api.getBossList(this.form)
				.then((r) => {
					if(r.status === '200') {
		...
		showDialogA(n, row) {
			...
			if (n === 0) {//add
				this.dialogA.form = {
					id: '', contentId: '',content: '', title: '', subtitle: '', 
					author: '', publicationTime: '', authorUrl: '',shareImgUrl: ''
				}
			} else{//modify
				this.dialogA.form = {
					id: row.id, 
					...
					shareImgUrl: row.shareImgUrl,
					contentId: row.contentId,
					content: row.contentResponse.content, 
				};
			};
			setTimeout(()=>{
		...
		handleAvatarSuccess2(res, file) {
			this.dialogA.form.shareImgUrl = this.upData.domain+res.key;
		},
		saveArticle() {
			if (!this.$com.getBtnClickDelay()) {return false};
			this.$api.bossAOM(this.dialogA.form)
				.then((r)=>{
					if (r.status === '200') {
		...
		deleteArt(n, row) {//n=0批量 1单个
			if (!n && !this.rowSelection.length) {
				this.$message.error('请先选择需要删除的文章！');
				return false
			};
			var ids = '';
			if (n) {
